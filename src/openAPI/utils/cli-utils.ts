...
export function logWithBorder(str: any, type = 'log') {
  var t: string = `--- ${str} ---`;
...
export function log(txt: string, type = 'log') {
  if (type === 'log') {
    console.log(txt);
  } else if (type === 'err') {
    console.error(txt);
...
export async function spinner(func: any, userMsg: string = '') {
...
export function toJsonPretty(obj: { cliStatus: number; msg: string; data: {}; }) {
  return JSON.stringify(obj, undefined, 2);
}

export function parseIfJSON(value: string) {
  try {
...
export async function progress(func: any, userMsg: string = '') {
  console.log(userMsg);
