...
import {ErrorMessageExt, systemType, textForCmd} from './textForCLIAndError';
            ...
            if (error) {
                if (stderr) {
                    if (stderr.toString().toLowerCase().includes("command not found")) {
                        reject(stderr.toString() + `.Install or Check if Akamai CLI is installed properly using the reference:${ErrorMessageExt.akamai_download_URI}`);
                    }
                    reject(stderr);
                } else {
                    reject('failure');
        ...
        exec(deleteFile, {}, (error: Error, stdout: string, stderr: string) => {
            if (error) {
                ...
                if (fs.existsSync(filePath)) {
                    throw new Error(ErrorMessageExt.file_replace_error + path.basename(filePath));
                }
                resolve('');
            }
        });
