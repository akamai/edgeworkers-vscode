    ...
    context.subscriptions.push(vscode.commands.registerCommand("edgeworkers-vscode.registerEdgeWorker", async function() {
        ...
        .then(async ()=> {
            await vscode.window.withProgress({
                ...
                try{
                    const groupIdsCmd= await akamiCLICalls.getEdgeWorkerListIds("edgeworkers","list-groups",path.resolve(os.tmpdir(),"akamaiCLIOutputListIds.json"));
                    const groupIds = await akamiCLICalls.executeAkamaiEdgeWorkerCLICmds(akamiCLICalls.generateCLICommand(groupIdsCmd),path.resolve(os.tmpdir(),"akamaiCLIOutputListIds.json"),"data");
                    const panel = vscode.window.createWebviewPanel(
                        'Register EdgeWorker',
                        'Register EdgeWorker',
                        vscode.ViewColumn.One,
